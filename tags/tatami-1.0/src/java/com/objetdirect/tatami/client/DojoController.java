...
public class DojoController {
	 ...
	 */
	private boolean require(String widgetDojo) {
		...
		if (!loaded) {
			...
			mapWidget.put(widgetDojo, new Boolean(true));
		}
		return !loaded;
	}
	...
	/*-{
	 ...
	 try {
	   ...
	   $wnd.dojo.hostenv.writeIncludes();
	 } catch (e) {
	...
	public void loadDojoWidget(HasDojo widget) {
		try {
			if (require("dojo.widget." + widget.getDojoName())) {
				widget.onDojoLoad();
			}
	 ...
	 * Creates a simple DOJO widget. Simple because no parameter are necessary to create it.
     * @param widget the DOJO widget to create. Example ! 
     *            <code>DojoController.getInstance().createSimple("Clock")</code>
     * @return the DOJO widget created.
     * @throws IllegalArgumentException if the creation failed.
	 */
	public JavaScriptObject createSimple(String widget) {
		JavaScriptObject dojo = null;
		try {
			dojo = createSimpleDojo(widget);
		} catch (Exception e) {
			throw new IllegalArgumentException("unknown Dojo widget : " + widget);
		}
		return dojo;
	}
	 ...
	 * Creates a simple DOJO widget from an interface <code>HasDojo</code>
	 * Calls the method {@link #createSimple(String)} with in parameter
	 * the result of the method {@link HasDojo#getDojoName()} to know the DOJO widget to create.
     * @param widget a widget implementing HasDojo
	 * @return the JavaScript corresponding to the DOJO widget.
	 */
	public JavaScriptObject createSimple(HasDojo widget) {
		return this.createSimple(widget.getDojoName());
	}
	 ...
	 * Creates a simple DOJO widget. Simple because no parameter are requiered. 
	 * @param widget the DOJO widget to create. Example 
     *            <code>DojoController.getInstance().createSimple("Clock")</code>
	 * @return the DOJO widget or null if problem
	 */
	private native JavaScriptObject createSimpleDojo(String widget)
	/*-{
	  try {
	       return $wnd.dojo.widget.createWidget(widget, {});
	  } catch (e) {
	       return null;    
	  }
