...
public class DateUtil {
	...
	public static String formatDate(Date date,String datePattern,String lang) 	{
		...
		return dojoDateFormat(theDate, datePattern, null, "dateOnly", lang);
	}
	...
	public static String formatTime(Date date,String timePattern,String lang){
		loadDojoDateUtil();
		JavaScriptObject theDate = getJSDate(date);
		return dojoDateFormat(theDate, null, timePattern, "timeOnly", lang);
	}
     ...
     * @param datePattern see the DOJO explanations : it's like in JAVA for the most of principals options 
     * @param timePattern see the DOJO explanations : it's like in JAVA for the most of principals options@param selector indique quel type de formatage est demandé : "dateOnly", "timeOnly" ou autre chose.
	...
	{
		loadDojoDateUtil();
		JavaScriptObject theDate = dojoDateParse(date, datePattern, null, "dateOnly", lang);
		if (theDate==null)
	 ...
	 */
	public static Date parseTime(
		String date,
		String timePattern,
		String lang)
	{
		loadDojoDateUtil();
		JavaScriptObject theDate = dojoDateParse(date, null, timePattern, "timeOnly", lang);
		if (theDate==null)
			throw new IllegalArgumentException(date);
		return getJavaDate(theDate);
	}
	...
	public static Date parseDateTime(String date,String datePattern,String timePattern,	String lang) 	{
		loadDojoDateUtil();
		JavaScriptObject theDate = dojoDateParse(date, datePattern, timePattern, "dateTime", lang);
		if (theDate==null) {
			throw new IllegalArgumentException(date);
		}
		return getJavaDate(theDate);
	...
	/**
	 * Loads if it's necessary the JavaScript scripts for DOJO to manipulate the dates.  
     * <p>
     * TODO : use the DojoController instead of this method
	 */
	static void loadDojoDateUtil() {
		if (!dojoDateUtil) {
			doLoadDojoDateUtil();
			dojoDateUtil = true;
		}
	...
	/**
	 * Loads the JavaScript scripts for DOJO to manipulate the dates.  
     * <p>
     * TODO : use the DojoController instead of this method
	 */
	static native void doLoadDojoDateUtil() 
	/*-{
		$wnd.dojo.require("dojo.date.format");
    	
	}-*/;
	
	/**
	 * TODO : Use the DojoController instead of this FLAG
	 * <p>
	 * Flag to indicate that the scripts JavaScript for DOJO to manage the date 
	 * was been loaded 
	 */		
	static boolean dojoDateUtil = false;	
