...
public class JamendoQueryMaker {
	...
	public static void dispatchJSON(JavaScriptObject jsonObj, int id){
		...
		Element scriptElem = DOM.getElementById("JamendoCallbackGeneratedScriptTag__"+id); 
	...
	public static  void searchGenre(JSONHandler handler){
		makeJSONRequest("http://api.jamendo.com/get2/id+idstr+weight/tag/jsoncallback/?order=rating_desc&n=50", handler,  null);
	}

	public static  void searchAlbumByArtist(JSONHandler handler  , ArtistItem parent) {
		makeJSONRequest("http://api.jamendo.com/get2/id+name+url+image+genre+duration+artist_name/album/jsoncallback/?artist_id="+SearchTree.getInstance().getStore().getIdentity(parent), handler,  parent);
	}

	public static void searchTracksByAlbum(JSONHandler handler , AlbumItem parent) {
		makeJSONRequest("http://api.jamendo.com/get2/track_id+track_name+track_url+track_duration+album_name+album_image+numalbum+artist_id+artist_name/track/jsoncallback/album_track+album_artist/?order=numalbum&album_id="+SearchTree.getInstance().getStore().getIdentity(parent), handler,  parent);
	}

	public static void searchArtistByLetter(JSONHandler handler , LetterItem parent){
		String[] letters = parent.getLetters();
		makeJSONRequest("http://api.jamendo.com/get2/id+name+url+image+artist_name/artist/jsoncallback/?n=50&pn="+parent.getNbChildPagesAlreadyLoaded()+"&artist_hasalbums=1&order=artist_name&artist_firstletter="+letters[parent.getCurrentLetterRequested()]+"&tag_idstr="+parent.getValue("genre", "unknown") , handler,  parent);
	}
