...
public class PlaylistGrid extends Grid implements LoadItemListener {
	...
	private PlaylistGrid(AbstractDataStore store) {
		...
		setMaximumFetchCountAtAtime(60);
	...
	public void addAlbumToPlaylist(AlbumItem album){
		...
		if(album.isFullyLoaded()){
			...
			SearchTree.getInstance().getStore().loadItem(album);
		}
	}
	
	public void addArtistToPlaylist(ArtistItem artist){
		List<?> tracks = (List<?>) artist.getChildren();
		if(artist.isFullyLoaded()){
			...
			SearchTree.getInstance().getStore().loadItem(artist);
		}
	}
