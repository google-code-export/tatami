...
public abstract class AbstractComponent extends AbstractProcessor implements 	MVCComponent {
	...
	public void addListener(MVCComponent _sl) {
		if (_sl != null) {
			...
			// else {
			// Category.getInstance(Constants.LOGGER_NAME).error("addListener::Wrong
			// Listener type "
			// + _sl.getClass().getName());
			// }
		}
	}
			...
			if (checkEventType(_e)) {
				if (listeners != null) {
					for (int i = listeners.size(); --i >= 0;) {
						MVCComponent l = (MVCComponent) listeners.get(i);
						l.process(_e);
					}
				}
			}
			// else {
			// Category.getInstance(Constants.LOGGER_NAME).error("fire::Wrong
			// Event type "+ _e.getClass().getName());
			// }
		}
	}
	...
	public void removeListener(MVCComponent _sl) {
		if (_sl != null) {
			...
			// else {
			// Category.getInstance(Constants.LOGGER_NAME).warn("removeListener:Wrong
			// Listener type "
			// + _sl.getClass().getName());
			// }
		}
